  ...
  describe '.find_for_facebook_oauth' do
    ...
    let(:access_token) do
      double(
        :access_token,
        provider: 'facebook',
        info: double(email: 'd.s.yurin@mail.ru'),
    ...
    context 'when user is not found' do
      it 'returns newly created user' do
        ...
        expect(user.email).to eq 'd.s.yurin@mail.ru'
      end
    end

    # Ситуация: юзер найден по почте
    context 'when user is found by email' do
      let!(:existing_user) { FactoryBot.create(:user, email: 'd.s.yurin@mail.ru') }
    ...
    context 'when user is found by provider + url' do
      let!(:existing_user) do
        FactoryBot.create(:user, provider: 'facebook',
                                 url: 'https://facebook.com/10213477015136709')
      end
      let!(:some_other_uer) { FactoryBot.create(:user) }
